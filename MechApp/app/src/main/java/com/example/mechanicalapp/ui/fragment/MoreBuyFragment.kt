...
class MoreBuyFragment : BaseCusFragment(), OnItemClickListener, View.OnClickListener,
    ...
    override fun onClick(p0: View?) {
        when (p0?.id) {
            ...
            R.id.tv_sure -> {
                (mPresenter as MecBuyPresenter)?.setScreen(mReScreenData)
                disScreenPop()
            }
            R.id.tv_reset ->   {
                (mPresenter as MecBuyPresenter)?.setScreen(null)
                mReScreenData =null
                mProgress1?.reset()
                mProgress2?.reset()
                mProgress3?.reset()
                ...
                PopUtils.init(mContext,
                    ...
                    ViewGroup.LayoutParams.MATCH_PARENT,true,false,object :PopUtils.onViewListener{
                        override fun getView(view: View?) {
                            ...
                            mProgress1?.addProgressListener(this@MoreBuyFragment)
                            mProgress2?.addProgressListener(this@MoreBuyFragment)
                            mProgress3?.addProgressListener(this@MoreBuyFragment)

                            mDialogTvRest?.setOnClickListener(this@MoreBuyFragment)
                            mDialogTvSure?.setOnClickListener(this@MoreBuyFragment)
                        }
